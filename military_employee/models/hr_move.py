...
class HrMove(models.Model):
    ...
    date = fields.Datetime(
        'Effective Date', default=fields.Datetime.now, index=True, required=True,
    ...
    product_uom_category_id = fields.Many2one(related='employee_id.uom_id.category_id')
    # TDE FIXME: make it stored, otherwise group will not work
    product_tmpl_id = fields.Many2one(
        'product.template', 'Product Template',
        related='employee_id.product_tmpl_id', readonly=False,
    ...
    procure_method = fields.Selection([
        ('make_to_stock', 'Default: Take From Hr'),
    ...
    rule_id = fields.Many2one(
        'stock.rule', 'Hr Rule', ondelete='restrict', help='The stock rule that created this stock move',
        check_company=True)
    ...
    has_tracking = fields.Selection(related='employee_id.tracking', string='Product with Tracking')
    ...
    product_type = fields.Selection(related='employee_id.type', readonly=True)
    ...
    @api.onchange('employee_id', 'picking_type_id')
    def onchange_product(self):
        if self.employee_id:
            product = self.employee_id.with_context(lang=self._get_lang())
    ...
    @api.depends('employee_id', 'has_tracking', 'move_line_ids')
    ...
    @api.depends('state', 'picking_id', 'employee_id')
    ...
    @api.depends('employee_id', 'product_uom', 'product_uom_qty')
    def _compute_product_qty(self):
        ...
        for move in self:
            move.product_qty = move.product_uom._compute_quantity(
                move.product_uom_qty, move.employee_id.uom_id, rounding_method=rounding_method)
    ...
    def _compute_reserved_availability(self):
        ...
        if not any(self._ids):
            ...
            for move in self:
                ...
                move.reserved_availability = move.employee_id.uom_id._compute_quantity(
    ...
    @api.depends('state', 'employee_id', 'product_qty', 'location_id')
    def _compute_product_availability(self):
        ...
        for move in self:
            if move.state == 'done':
                ...
                total_availability = self.env['stock.quant']._get_available_quantity(move.employee_id, move.location_id) if move.employee_id else 0.0
                move.availability = min(move.product_qty, total_availability)

    @api.depends('employee_id', 'picking_type_id', 'picking_id', 'reserved_availability', 'priority', 'state', 'product_uom_qty', 'location_id')
    def _compute_forecast_information(self):
        ...
        not_product_moves = self.filtered(lambda move: move.employee_id.type != 'product')
        ...
        for move in product_moves:
            ...
            if picking_type.code in self._consuming_picking_types() and is_unreserved:
                ...
                move.forecast_availability = move.product_uom._compute_quantity(
                    move.reserved_availability, move.employee_id.uom_id, rounding_method='HALF-UP')
        ...
        for warehouse, moves in outgoing_unreserved_moves_per_warehouse.items():
            if not warehouse:  # No prediction possible if no warehouse.
                continue
            product_variant_ids = moves.employee_id.ids
    ...
    def _set_lot_ids(self):
        for move in self:
            if move.employee_id.tracking != 'serial':
                continue
            move_lines_commands = []
            ...
            for lot in move.lot_ids:
                if lot not in ls:
                    ...
                    move_line_vals['product_uom_id'] = move.employee_id.uom_id.id
    ...
    def _check_uom(self):
        moves_error = self.filtered(lambda move: move.employee_id.uom_id.category_id != move.product_uom.category_id)
        if moves_error:
            ...
            for move in moves_error:
                user_warning += _('\n\n%s --> Product UoM is %s (%s) - Move UoM is %s (%s)') % (move.employee_id.display_name, move.employee_id.uom_id.name, move.employee_id.uom_id.category_id.name, move.product_uom.name, move.product_uom.category_id.name)
    ...
    def init(self):
        ...
        if not self._cr.fetchone():
            self._cr.execute('CREATE INDEX stock_move_product_location_index ON stock_move (employee_id, location_id, location_dest_id, company_id, state)')
    ...
    def default_get(self, fields_list):
        ...
        # auto-confirmed.
        defaults = super(HrMove, self).default_get(fields_list)
        if self.env.context.get('default_picking_id'):
    ...
    def name_get(self):
        ...
        for move in self:
            res.append((move.id, '%s%s%s>%s' % (
                ...
                move.employee_id.code and '%s: ' % move.employee_id.code or '',
    ...
    def action_product_forecast_report(self):
        self.ensure_one()
        action = self.employee_id.action_product_forecast_report()
    ...
    def _push_apply(self):
        for move in self:
            ...
            if move.location_dest_id.company_id == self.env.company:
                rules = self.env['procurement.group']._search_rule(move.route_ids, move.employee_id, warehouse_id, domain)
            else:
                rules = self.sudo().env['procurement.group']._search_rule(move.route_ids, move.employee_id, warehouse_id, domain)
    ...
    def _prepare_merge_moves_distinct_fields(self):
        return [
            'employee_id', 'price_unit', 'procure_method', 'location_id', 'location_dest_id',
    ...
    def _prepare_merge_move_sort_method(self, move):
        ...
        return [
            move.employee_id.id, move.price_unit, move.procure_method, move.location_id, move.location_dest_id,
    ...
    def _onchange_lot_ids(self):
        ...
        quantity_done += self.employee_id.uom_id._compute_quantity(len(self.lot_ids), self.product_uom)
    ...
    def onchange_product_uom(self):
        if self.product_uom.factor > self.employee_id.uom_id.factor:
            return {
                'warning': {
    ...
    def _generate_serial_move_line_commands(self, lot_names, origin_move_line=None):
        ...
        if origin_move_line:
            ...
            location_dest = self.location_dest_id._get_putaway_strategy(self.employee_id)
        move_line_vals = {
            'picking_id': self.picking_id.id,
            'location_dest_id': location_dest.id or self.location_dest_id.id,
            'location_id': self.location_id.id,
            'employee_id': self.employee_id.id,
            'product_uom_id': self.employee_id.uom_id.id,
            'qty_done': 1,
        }
        if origin_move_line:
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        for move in move_create_proc:
            ...
            procurement_requests.append(self.env['procurement.group'].Procurement(
                move.employee_id, move.product_uom_qty, move.product_uom,
    ...
    def _prepare_procurement_values(self):
        ...
        if self.rule_id:
            if self.rule_id.group_propagation_option == 'fixed' and self.rule_id.group_id:
                ...
                group_id = False
        employee_id = self.employee_id.with_context(lang=self._get_lang())
        return {
            'product_description_variants': self.description_picking and self.description_picking.replace(employee_id._get_description(self.picking_type_id), ''),
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        self.ensure_one()
        # apply putaway
        location_dest_id = self.location_dest_id._get_putaway_strategy(self.employee_id).id or self.location_dest_id.id
        vals = {
            'move_id': self.id,
            'employee_id': self.employee_id.id,
        ...
        if quantity:
            ...
            uom_quantity = self.employee_id.uom_id._compute_quantity(quantity, self.product_uom, rounding_method='HALF-UP')
            uom_quantity = float_round(uom_quantity, precision_digits=rounding)
            uom_quantity_back_to_product_uom = self.product_uom._compute_quantity(uom_quantity, self.employee_id.uom_id, rounding_method='HALF-UP')
            if float_compare(quantity, uom_quantity_back_to_product_uom, precision_digits=rounding) == 0:
                vals = dict(vals, product_uom_qty=uom_quantity)
            else:
                vals = dict(vals, product_uom_qty=quantity, product_uom_id=self.employee_id.uom_id.id)
        if reserved_quant:
            vals = dict(
                vals,
    ...
    def _update_reserved_quantity(self, need, available_quantity, location_id, lot_id=None, package_id=None, owner_id=None, strict=True):
        ...
        if not strict and self.employee_id.uom_id != self.product_uom:
            taken_quantity_move_uom = self.employee_id.uom_id._compute_quantity(taken_quantity, self.product_uom, rounding_method='DOWN')
            taken_quantity = self.product_uom._compute_quantity(taken_quantity_move_uom, self.employee_id.uom_id, rounding_method='HALF-UP')

        quants = []
        rounding = self.env['decimal.precision'].precision_get('Product Unit of Measure')

        if self.employee_id.tracking == 'serial':
            if float_compare(taken_quantity, int(taken_quantity), precision_digits=rounding) != 0:
                taken_quantity = 0

        try:
            with self.env.cr.savepoint():
                if not float_is_zero(taken_quantity, precision_rounding=self.employee_id.uom_id.rounding):
                    quants = self.env['stock.quant']._update_reserved_quantity(
                        self.employee_id, location_id, taken_quantity, lot_id=lot_id,
        ...
        for reserved_quant, quantity in quants:
            ...
            if to_update:
                uom_quantity = self.employee_id.uom_id._compute_quantity(quantity, to_update.product_uom_id, rounding_method='HALF-UP')
                uom_quantity = float_round(uom_quantity, precision_digits=rounding)
                uom_quantity_back_to_product_uom = to_update.product_uom_id._compute_quantity(uom_quantity, self.employee_id.uom_id, rounding_method='HALF-UP')
    ...
    def _should_bypass_reservation(self):
        self.ensure_one()
        return self.location_id.should_bypass_reservation() or self.employee_id.type != 'product'
    ...
    def _get_available_quantity(self, location_id, lot_id=None, package_id=None, owner_id=None, strict=False, allow_negative=False):
        self.ensure_one()
        return self.env['stock.quant']._get_available_quantity(self.employee_id, location_id, lot_id=lot_id, package_id=package_id, owner_id=owner_id, strict=strict, allow_negative=allow_negative)
    ...
    def _action_assign(self):
        ...
        partially available.
        """
        HrMove = self.env['stock.move']
        assigned_moves_ids = OrderedSet()
        partially_available_moves_ids = OrderedSet()
        # Read the `reserved_availability` field of the moves out of the loop to prevent unwanted
        # cache invalidation when actually reserving the move.
        reserved_availability = {move: move.reserved_availability for move in self}
        roundings = {move: move.employee_id.uom_id.rounding for move in self}
        move_line_vals_list = []
        for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
            rounding = roundings[move]
            missing_reserved_uom_quantity = move.product_uom_qty - reserved_availability[move]
            missing_reserved_quantity = move.product_uom._compute_quantity(missing_reserved_uom_quantity, move.employee_id.uom_id, rounding_method='HALF-UP')
            if move._should_bypass_reservation():
                # create the move line(s) but do not impact quants
                if move.employee_id.tracking == 'serial' and (move.picking_type_id.use_create_lots or move.picking_type_id.use_existing_lots):
                ...
                if float_is_zero(move.product_uom_qty, precision_rounding=move.product_uom.rounding):
                    ...
                    # `employee_id.uom_id` (the UOM of the quants is the UOM of the product).
                    ...
                    for k, g in groupby(sorted(move_lines_in, key=_keys_in_sorted), key=itemgetter(*keys_in_groupby)):
                        qty_done = 0
                        for ml in g:
                            qty_done += ml.product_uom_id._compute_quantity(ml.qty_done, ml.employee_id.uom_id)
                    ...
                    moves_out_siblings_to_consider = moves_out_siblings & (HrMove.browse(assigned_moves_ids) + HrMove.browse(partially_available_moves_ids))
                    ...
                    rounding = move.employee_id.uom_id.rounding
        ...
        HrMove.browse(partially_available_moves_ids).write({'state': 'partially_available'})
        HrMove.browse(assigned_moves_ids).write({'state': 'assigned'})
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        for move in moves_todo:
            ...
            if float_compare(move.quantity_done, move.product_uom_qty, precision_digits=rounding) < 0:
                ...
                qty_split = move.product_uom._compute_quantity(move.product_uom_qty - move.quantity_done, move.employee_id.uom_id, rounding_method='HALF-UP')
    ...
    def _split(self, qty, restrict_partner_id=False):
        ...
        if float_is_zero(qty, precision_rounding=self.employee_id.uom_id.rounding) or self.product_qty <= qty:
            return []
        ...
        uom_qty = self.employee_id.uom_id._compute_quantity(qty, self.product_uom, rounding_method='HALF-UP')
        if float_compare(qty, self.product_uom._compute_quantity(uom_qty, self.employee_id.uom_id, rounding_method='HALF-UP'), precision_digits=decimal_precision) == 0:
            defaults = self._prepare_move_split_vals(uom_qty)
        else:
            defaults = self.with_context(force_split_uom_id=self.employee_id.uom_id.id)._prepare_move_split_vals(qty)
        ...
        new_product_qty = self.employee_id.uom_id._compute_quantity(self.product_qty - qty, self.product_uom, round=False)
    ...
    def _get_upstream_documents_and_responsibles(self, visited):
        if self.move_orig_ids and any(m.state not in ('done', 'cancel') for m in self.move_orig_ids):
            ...
            return [(self.picking_id, self.employee_id.responsible_id, visited)]
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        if float_compare(qty, 0.0, precision_rounding=self.product_uom.rounding) > 0:
            if self.employee_id.tracking != 'serial':
                ...
                uom_qty = self.product_uom._compute_quantity(qty, self.employee_id.uom_id)
                for i in range(0, int(uom_qty)):
                    vals = self._prepare_move_line_vals(quantity=0)
                    vals['qty_done'] = 1
                    vals['product_uom_id'] = self.employee_id.uom_id.id
                    res.append((0, 0, vals))
        return res
    ...
    def _adjust_procure_method(self):
        ...
        for move in self:
            ...
            rules = self.env['procurement.group']._search_rule(False, employee_id, move.warehouse_id, domain)
            if rules:
                if rules.procure_method in ['make_to_order', 'make_to_stock']:
                    ...
                    mtso_needed_qties_by_loc[rules.location_src_id].setdefault(employee_id.id, 0)
                    mtso_needed_qties_by_loc[rules.location_src_id][employee_id.id] += move.product_qty

                    # This allow us to get the forecasted quantity in batch later on
                    mtso_products_by_locations[rules.location_src_id].append(employee_id.id)
                    mtso_moves |= move
            else:
                move.procure_method = 'make_to_stock'

        # Get the forecasted quantity for the `mts_else_mto` moves.
        for location, employee_ids in mtso_products_by_locations.items():
            products = self.env['hr.employee'].browse(employee_ids).with_context(location=location.id)
        ...
        for move in mtso_moves:
            ...
            forecasted_qty = mtso_free_qties_by_loc[move.location_id][move.employee_id.id]
            if float_compare(needed_qty, forecasted_qty, precision_rounding=employee_id.uom_id.rounding) <= 0:
                move.procure_method = 'make_to_stock'
                mtso_free_qties_by_loc[move.location_id][move.employee_id.id] -= needed_qty
            else:
    ...
    def _trigger_assign(self):
        ...
        domains = []
        for move in self:
            domains.append([('employee_id', '=', move.employee_id.id), ('location_id', '=', move.location_dest_id.id)])
