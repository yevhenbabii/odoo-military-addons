...
class HrLocation(models.Model):
    _name = "hr.location"
    _description = "Employee Locations"
    ...
    def default_get(self, fields):
        res = super(HrLocation, self).default_get(fields)
    ...
    usage = fields.Selection([
        ...
        help="* Partner Location: Virtual location representing the source location for employees coming from your partners"
             "\n* View: Virtual location used to create a hierarchical structures for your warehouse, aggregating its child locations ; can't directly contain products"
             "\n* Internal Location: Physical locations inside your own warehouses,"
             "\n* External Lo"
             "\n* Loss: Virtual location serving as counterpart for employee losses")
    location_id = fields.Many2one(
        'hr.location', 'Parent Location', index=True, ondelete='cascade', check_company=True,
        help="The parent location that includes this location. Example : The 'Dispatch Zone' is the 'Gate 1' parent location.")
    child_ids = fields.One2many('hr.location', 'location_id', 'Contains')
    ...
    quant_ids = fields.One2many('hr.quant', 'location_id')
    ...
    def write(self, values):
        ...
        if 'usage' in values or 'scrap_location' in values:
            ...
            reserved_quantities = self.env['hr.move.line'].search_count([
        ...
        if 'active' in values:
            ...
            if not self.env.context.get('do_not_check_quant'):
                children_location = self.env['hr.location'].with_context(active_test=False).search([('id', 'child_of', self.ids)])
    ...
    def _get_putaway_strategy(self, product):
        ...
        putaway_location = self.env['hr.location']
